...
child_class2 *init_child_class2 (const char * const specific, const int field)
{
    int specific_length;
    child_class2 *child_tmp;
    parent_class *parent_tmp;

    parent_tmp = init_parent_class(field);

    if (!(child_tmp = (child_class2 *)malloc(sizeof(struct _child_class2))))
    {
    ...
    child_tmp->control = control_child2_field;
    child_tmp->free = free_child2_class;
    child_tmp->print = print_child2_class;

    return child_tmp;
}

void control_child2_field (child_class2 * const obj, const int field, const char * const specific)
{
...
void free_child2_class (child_class2 * const obj)
{
    obj->base_obj->free(obj->base_obj);
    free(obj);
}

void print_child2_class (child_class2 * const obj)
{
